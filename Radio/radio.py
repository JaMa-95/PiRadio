...
class Radio:
    ...
    def check_shutdown_raspi(self):
        if self.radio_buttons.on_off_raspi_button.active:
            if self.radio_buttons.on_off_raspi_button.long_click():
                print(f"on off has changed: {self.radio_buttons.on_off_button.state}")
                time.sleep(4)
                self.raspberry.turn_raspi_off()
    ...
    def check_radio_lock(self):
        if self.radio_buttons.frequency_lock_button.active:
            if self.radio_buttons.frequency_lock_button.state != self.radio_lock:
                self.radio_lock = self.radio_buttons.frequency_lock_button.state
